...
env_init(void)
{
    ...
    LIST_INIT(&env_sched_list[0]);// lab4->lab3
    LIST_INIT(&env_sched_list[1]);// lab4->lab3
...
env_setup_vm(struct Env *e)
{
     ...
     */
    bcopy(&boot_pgdir[PDX(UTOP)],&pgdir[PDX(UTOP)],1024-PDX(UTOP));
    //for (i = PDX(UTOP);i < 1024;i++)
	//pgdir[i] = boot_pgdir[i];
    e->env_pgdir = pgdir;
    e->env_cr3 = PADDR(pgdir);
    // UVPT maps the env's own page table, with read-only permission.
    e->env_pgdir[PDX(UVPT)]  = e->env_cr3 | PTE_V|PTE_R; // lab4 -> lab3
...
env_alloc(struct Env **new, u_int parent_id)
{
	...
	*new = e; //lab4->lab3
    ...
    LIST_INSERT_HEAD(&env_sched_list[0],e,env_sched_link);  // lab4 -> lab3
    *new = e;
    return 0;
                             ...
                             u_char *bin, u_int32_t bin_size, void *user_data)
{
    ...
    //printf("env.c/load_icode_mapper->va:0x%x\n",va);
    /*Step 1: load all content of bin into memory. */
    for (i = 0; i < bin_size; i += BY2PG) {
        /* Hint: You should alloc a new page. */
	if (BY2PG + i - offset < bin_size && i ==0) 
		len = BY2PG - offset;
        else if (BY2PG + i < bin_size) 
		len = BY2PG;
	else 
		len = bin_size - i; 
	if(page_alloc(&p)==0) {
	    // p->pp_ref++;
	   if (i ==0) {
	         bcopy(bin,(char *)page2kva(p)+offset,len);
	   }
	   else { 
	        bcopy(bin+i-offset,(char *)page2kva(p),len);
	    }
    ...
    * hint: variable `i` has the value of `bin_size` now! */
    while (i < sgsize) {
	 if(!page_alloc(&p)==0) 
		return -1;
	/// p->pp_ref+=1; //page_insert has p->pp_ref++;
...
load_icode(struct Env *e, u_char *binary, u_int size)
{
   ...
   // perm = PTE_R | PTE_V;
	perm = PTE_R; //lab4 -> lab3
    r = page_insert(e->env_pgdir,p,USTACKTOP-BY2PG,perm);
    if (r != 0)
	return;

   if( load_elf(binary,size,&entry_point,(void *)e,load_icode_mapper)){
	printf("load_elf failed at line 324\n");
	return;
    }    
...
env_create_priority(u_char *binary, int size, int priority)
{
    ...
    if(env_alloc(&e,0)<0) {
	printf("env_alloc failed at line 348\n"); 
	return;
    }
...
env_run(struct Env *e)
{
    ...
    *  switch the context and save the registers. You can imitate env_destroy() 's behaviors.*/
	...
	bcopy(old,&(curenv->env_tf),sizeof(struct Trapframe));
    	curenv->env_tf.pc = curenv->env_tf.cp0_epc;//lab4->lab3
    }
    /*Step 2: Set 'curenv' to the new environment. */
    curenv = e;
    //curenv->env_status = ENV_RUNNABLE;//lab4->lab3
    curenv->env_runs++;//lab4->lab3
