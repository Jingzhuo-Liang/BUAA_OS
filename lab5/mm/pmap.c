...
static Pte *boot_pgdir_walk(Pde *pgdir, u_long va, int create)
{
	...
	if (create == 1) {
		pgtable = alloc(BY2PG,BY2PG,1); // pgtable is a physical address
...
void boot_map_segment(Pde *pgdir, u_long va, u_long size, u_long pa, int perm)
{
    int i;
    Pte *pgtable_entry;
    u_long va_temp = va, pa_temp = pa;
    /* Step 1: Check if `size` is a multiple of BY2PG. */
//	printf("I'm here\n");
    if ((size % BY2PG) != 0)
	return;
    u_long num = size / BY2PG;
...
page_init(void)
{
    ...
    /* Step 2: Align `freemem` up to multiple of BY2PG(=4096). */
...
page_alloc(struct Page **pp)
{
    ...
    *pp = ppage_temp;
    ...
    bzero((void *)page2kva(*pp),BY2PG);
          // page2kva ----> get the kernel virtual address of page'PP'			
     return 0;
...
pgdir_walk(Pde *pgdir, u_long va, int create, Pte **ppte)
{
	...
	if (create == 1) {
		if (page_alloc(&ppage)==0) {	
			ppage->pp_ref += 1;
			pgtable = KADDR(page2pa(ppage));   // get ppage's physical address then get ppage's virtual
 								//address
			*pgdir_entryp = (page2pa(ppage)|PTE_V); // set physical address and valid bit in pgdir_entry	
		}
		else 
	    		return -E_NO_MEM;
    	}
