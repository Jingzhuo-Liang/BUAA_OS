...
pgfault(u_int va)
{
	...
	u_int perm = ((Pte *)*vpt)[va / BY2PG];
	if ((perm & PTE_COW) == 0) {
		user_panic("in fork.c, PTE_COW is not valid");
	}
	tmp = USTACKTOP;
	va = ROUNDDOWN(va,BY2PG);
	syscall_mem_alloc(0,tmp,PTE_V | PTE_R);
	user_bcopy(va,tmp,BY2PG);
	syscall_mem_map(0,tmp,0,va,PTE_R | PTE_V);
